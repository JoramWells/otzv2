...
const Messages = ({patientID}:{patientID:string}) => {
    const {data, isLoading} = useGetPatientNotificationQuery(patientID);
    console.log(data)
  ...
  return (
    <div className="flex flex-col items-center space-y-4 w-full">
      {data?.map((item: any) => (
        ...
        >
          {item.message}
          {isSame(item.medicineTime, moment(item.updateAt, "HH:mm:ss"))
            ? "same"
            : "not same"}
          <div>
            <p>Date: {moment(item.updateAt).format('ll')} </p>
