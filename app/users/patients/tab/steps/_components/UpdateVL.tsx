...
const UpdateVL = ({ handleBack, handleNext, patientID, patientVisitID }: InputProps) => {
  const [useAddPatientMutation] = useAddViralLoadTestMutation()
  ...
  const statusOptions = useCallback(() => {
    return statusData?.filter(item => (item.statusDescription.toLowerCase() === 'upcoming'))
  }, [statusData])

  //
  const agendaDataOptions = useCallback(() => {
    return agendaData?.filter(
      (item) => item.agendaDescription.toLowerCase() === 'viral load'
    )
  }, [agendaData])
